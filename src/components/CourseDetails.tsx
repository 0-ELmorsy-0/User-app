            ...
            if (!modsError && modsData) {
              const mapped = subsData.map((s: any) => {
                ...
                const subjectModules = modsData
                  ...
                  .map((m: any) => {
                    const moduleItems = Array.isArray(m.items) ? m.items.map((i: any) => ({
                      title: i.title,
                      url: i.url,
                    ...
                    moduleAssignments.forEach((assignment: any) => {
                      moduleItems.push({
                        ...
                        isExam: true, // Both open the exam modal
                ...
                <div className="flex flex-col items-center mb-8">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/50 w-full max-w-sm mt-6 p-4 rounded-xl text-sm border border-slate-100 dark:border-slate-700/50 space-y-3">
                    ...
                    <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                      ...
                      <span className="bg-[#4285f4] text-white text-xs font-bold w-6 h-6 flex items-center justify-center rounded-md">{Object.keys(selectedAnswers).length}</span>
                    </div>
                  </div>
